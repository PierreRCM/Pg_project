...
from image_data import init_image, image_data_original, xp_per_level
pg.init()
...
class Bullet(pg.sprite.Sprite):
    ...
    def __init__(self, name, direction, position, range_, damage, speed, tick):
        ...
        self.flags = {"hit": False, "alive": True, "border": False}
        self._rotate()
    ...
    def _move(self):
        ...
        self._attr["position"][0] += cos((self._attr["direction"] + 90) * pi / 180) * self._attr["speed"] * self._attr["tick"]
        self._attr["position"][1] += sin(-(self._attr["direction"] + 90) * pi / 180) * self._attr["speed"] * self._attr["tick"]
...
class Player(pg.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self._attr = {"reference": 270, "position": [80, 80], "old_position": [10, 10], "speed": 180, "hp": 100,
                      "mouse": [0, 0], "direction": 0, "tick": pg.time.Clock().tick(60)/1000, "money": 0,
                      "hp_max": 100, "damage": self.weapon.damage, "rate": self.weapon.rate, "level": 1, "xp": 0}

        self.flags = {"hit": False, "levelup": False, "xp_earn": False, "alive": True, "border": False, "fire": False,
                      "shot_ready": True}
        self.all_bonus = []
        self.keys = pg.key.get_pressed()
        pg.sprite.Sprite.__init__(self)

    def handle_xp(self, xp):

        global xp_per_level

        self._attr["xp"] += xp
        self.flags["xp_earn"] = True

        if self._attr["xp"] >= xp_per_level[str(self._attr["level"]+1)]:

            self._attr["xp"] -= xp_per_level[str(self._attr["level"]+1)]
            self._attr["level"] += 1
            self.flags["levelup"] = True
    ...
    def _cooldown_shot(self):
        ...
        if (now - self.last_shot)/1000 >= (1/self._attr["rate"]):
            ...
            self.flags["shot_ready"] = True

        else:

            self.flags["shot_ready"] = False
    ...
    def _rotate(self):
        ...
        self._attr["direction"] = atan2(deltax, deltay) * 180 / pi + self._attr["reference"]
...
class Enemy(pg.sprite.Sprite):  # TODO: Create an entity class, and different enemy, different IA
    ...
    def __init__(self, player, carac, clock):
        ...
        self._attr = {"reference": 270, "position": [carac["pos"][0], carac["pos"][1]], "old_position": [10, 10],
                      "speed": carac["speed"], "hp": carac["hp"], "direction": 0,
                      "tick": carac["tick"], "rate": carac["rate"], "damage": carac["damage"],
                      "range": carac["range"], "shot_speed": carac["shot_speed"],
                      "bonus": carac["bonus"], "direc": "LEFT", "xp": carac["xp"]}

        self.flags = {"hit": False, "alive": True, "border": False, "fire": False, "shot_ready": True,
                      "bonus_bool": carac["bonus_bool"]}
        self.index_image = 0
        self.display_time = 0
        pg.sprite.Sprite.__init__(self)
    ...
    def _decision(self):
        ...
        for dist in distance:
            ...
            norm.append(sqrt((-t_pos_x + e_pos_x + dist[0])**2 + (-t_pos_y + e_pos_y + dist[1])**2))
    ...
    def _shoot(self):
        ...
        norm = sqrt((self.target.get_attr("position")[0] - self._attr["position"][0])**2 +
